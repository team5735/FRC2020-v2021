 ...
 * @author Jaci
 */
public class ReverseEncoderFollower implements CFEncoderFollower{
    ...
    double last_error, heading, finalSegPosition;

    int segment;
    Trajectory trajectory;

    public ReverseEncoderFollower(Trajectory traj) {
        this.trajectory = traj;
        finalSegPosition = trajectory.get(trajectory.length() - 1).position;
    }

    public ReverseEncoderFollower() { }
    ...
    public double calculate(int encoder_tick) {
        ...
        if (segment < trajectory.length()) {
            Trajectory.Segment seg = trajectory.get((trajectory.length() - 1) - segment); // get segment starting from back
            double error = finalSegPosition - seg.position - distance_covered;
