...
public class TrajectoryUtil {
    ...
    public static <S extends IPose2d<S>> Trajectory<S> transform(final Trajectory<S> trajectory, Pose transform) {
        List<S> waypoints = new ArrayList<>(trajectory.length());
        for (int i = 0; i < trajectory.length(); ++i) {
    ...
    public static Trajectory<PoseWithCurvature> trajectoryFromPathFollower(IPathFollower path_follower,
                                                                             PoseWithCurvature start_state, double
                                                                                     step_size, double
                                                                                     dcurvature_limit) {
        List<PoseWithCurvature> samples = new ArrayList<PoseWithCurvature>();
        samples.add(start_state);
        PoseWithCurvature current_state = start_state;
        while (!path_follower.isDone()) {
            // Get the desired steering command.
            final Twist raw_steering_command = path_follower.steer(current_state.getPose());

            // Truncate to the step size.
            Twist steering_command = raw_steering_command;
            ...
            if (dcurvature > dcurvature_limit && curvature_valid) {
                steering_command = new Twist(steering_command.dx, steering_command.dy,
            ...
            final Twist average_steering_command = !curvature_valid
                    ? steering_command
                    : new Twist(steering_command.dx, steering_command.dy,
                    (current_state.getCurvature() + 0.5 * dcurvature * steering_command.norm())
                            * steering_command.norm());
            current_state = new PoseWithCurvature(
                    current_state.getPose().transformBy(Pose.exp(average_steering_command)),
        ...
        return new Trajectory<PoseWithCurvature>(samples);
    }

    public static Trajectory<PoseWithCurvature> trajectoryFromSplineWaypoints(final List<Pose> waypoints, double
    ...
    public static Trajectory<PoseWithCurvature> trajectoryFromSplines(final List<? extends Spline> splines, double
            maxDx,
