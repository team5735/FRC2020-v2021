...
public class TrajectoryGenerator {
    ...
    public static Trajectory[] generateTrajectoryWithWaypoints(Waypoint[] points) {
        ...
        Trajectory.Config config = new Trajectory.Config(Trajectory.FitMethod.HERMITE_CUBIC, Trajectory.Config.SAMPLES_HIGH, 0.02, 0.69 * RobotConstants.MAX_VELOCITY_DT, 0.8, 8);
    ...
    public static Trajectory[] readTrajectoriesNew(String fileName) {
        try {
            File leftTraj = Filesystem.getDeployDirectory().toPath().resolve(fileName + "_left.csv").toFile();
            File rightTraj = Filesystem.getDeployDirectory().toPath().resolve(fileName + "_right.csv").toFile();
